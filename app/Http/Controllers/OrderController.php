...
class OrderController extends Controller
{
	...
	public function MISOverview(Request $request){
        ...
        //$query->whereDate('c_created_at', '=', Carbon::parse($currentDate)->format('Y-m-d'));

        $data['total_no'] = Orders::whereDate('c_created_at', '=', Carbon::parse($currentDate)->format('Y-m-d'))->count();

        $data['total_amt'] = Orders::whereDate('c_created_at', '=', Carbon::parse($currentDate)->format('Y-m-d'))->sum(DB::raw('total_price - shipping_charges'));

        $data['phonepe'] = Orders::where([ ['bap_id', 'like','%shopping-network.phonepe.com%'],['c_created_at', 'like', $currentDate.'%'] ])->whereDate('c_created_at', '=', Carbon::parse($currentDate)->format('Y-m-d'))->get()->count();

        $data['phonepe_amt'] = Orders::where('bap_id', 'like','%shopping-network.phonepe.com%')->whereDate('c_created_at', '=', Carbon::parse($currentDate)->format('Y-m-d'))->sum(DB::raw('total_price - shipping_charges'));

        $data['paytm'] = Orders::where([ ['bap_id', 'like','%ondc.paytm.com%'],['c_created_at', 'like', $currentDate.'%'] ])->whereDate('c_created_at', '=', Carbon::parse($currentDate)->format('Y-m-d'))->get()->count();

        $data['paytm_amt'] = Orders::where([ ['bap_id', 'like','%ondc.paytm.com%'],['c_created_at', 'like', $currentDate.'%'] ])->sum(DB::raw('total_price - shipping_charges'));

        $data['magicpin'] = Orders::where([ ['bap_id', 'like','%webapi.magicpin.in%'],['c_created_at', 'like', $currentDate.'%'] ])->whereDate('c_created_at', '=', Carbon::parse($currentDate)->format('Y-m-d'))->get()->count();

        $data['magicpin_amt'] = Orders::where([ ['bap_id', 'like','%webapi.magicpin.in%'],['c_created_at', 'like', $currentDate.'%'] ])->sum(DB::raw('total_price - shipping_charges'));

        $data['nobrokerhood'] = Orders::where([ ['bap_id', 'like','%nobrokerhood%'],['c_created_at', 'like', $currentDate.'%'] ])->whereDate('c_created_at', '=', Carbon::parse($currentDate)->format('Y-m-d'))->get()->count();

        $data['nobrokerhood_amt'] = Orders::where([ ['bap_id', 'like','%nobrokerhood%'],['c_created_at', 'like', $currentDate.'%'] ])->sum(DB::raw('total_price - shipping_charges'));

        $start_date = date('Y-m-d',strtotime("last Monday"));
        $end_date = date('Y-m-d', strtotime($start_date . ' + 6 days'));

        // Query for MIS overview data
       $result = Orders::selectRaw('date(c_created_at) as date, std_code, 
                ...
                ->whereBetween(DB::raw('date(c_created_at)'), [$start_date, $end_date])
                ->groupBy(DB::raw('date(c_created_at)'), 'std_code')
                ->orderBy('date', 'desc')
                ->get();
     ...
     public function downloadCSV(Request $request)
    {
       ...
       $result = Orders::selectRaw('date(c_created_at) as date, std_code, 
                ...
                SUM(IF(bap_id like "%nobrokerhood%" AND order_status="Cancelled", 1, 0)) AS nobrokerhood_cancelled_orders,
                SUM(IF(bap_id like "%nobrokerhood%" AND order_status="Cancelled", total_price, 0)) AS nobrokerhood_cancelled_amt')
                ->whereBetween(DB::raw('date(c_created_at)'), ['2023-10-01', '2023-10-11'])
                ->groupBy(DB::raw('date(c_created_at)'), 'std_code')
                ->get();
